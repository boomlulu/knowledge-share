...
<h1>难排的地方在于：主链路正常，偏差藏在支路里</h1>

<h3>它为什么不容易第一时间暴露</h3>

- 主流程状态是稳定的，测试时很容易先相信它没问题
- 偏差通常从很小的字段开始，不会立刻传导到表现层
- 常规验收更关注画面和手感，不一定覆盖每一条逻辑支路
- 真正暴露出来时，往往已经不是第一帧偏差，而是后续扩散结果

<div class="callout warning">
所以这类问题难的不是“知道哪里用了 float”，而是找到第一帧、第一条支路、第一处状态偏差。
</div>
...
<h1>这个项目的风险，在于太多系统参与逻辑结果</h1>

<div class="three-card-grid">
  <div class="mini-card">
    <h3>游戏形态</h3>
    <p>射击 MOBA，双线对战，小兵、敌人、防御塔都会参与战斗判断。</p>
  </div>
  <div class="mini-card">
    <h3>同步路线</h3>
    <p>采用帧同步，客户端需要本地推进完整战斗逻辑。</p>
  </div>
  <div class="mini-card">
    <h3>底层依赖</h3>
    <p>Unity `float`、物理、寻路、动画结果都可能影响逻辑分支。</p>
  </div>
</div>

<div class="callout danger">
这些模块不是不能用，而是不适合直接承担“所有客户端每帧逻辑结果完全一致”的要求。
</div>
...
<h1>服务端权威降低的是客户端确定性压力</h1>

<h3>它解决的是哪一部分压力</h3>

- 不再要求每个客户端完整、严格地模拟出同一份逻辑状态
- 客户端即使有预测、插值、延迟修正，最终也可以收敛到服务端权威状态
- 所以它绕开了一部分“所有客户端逐帧绝对一致”的压力

<div class="callout">
它解决了客户端确定性压力，但没有消灭同步复杂度。
</div>
...
<div class="callout">
它承认偏差可能发生，因此把工程压力从“永不分叉”转到“可检测、可收敛”。
</div>
